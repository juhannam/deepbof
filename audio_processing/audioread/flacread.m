...
if strcmp(FMT,'size') == 1
  ...
  % Load the data, if the tmpfile succeeded in being written
  if exist(tmpfile,'file') > 0
	[Y,SR] = wavread_downsamp(tmpfile, [], forcemono, downsamp);
	% Delete tmp file
  	mysystem([rmcmd,' "', tmpfile,'"']);
  else
    disp(['warn: unable to read temp file ',tmpfile]);
    Y = [];
  end	
...
[s,w] = system(cmd);
if s ~= 0 
  % Do we stop if there's a problem with flac, or just continue?
  %error(['unable to execute ',cmd,' (',w(1:end-1),')']);
  disp(['warn: error running ',cmd,' (',w(1:end-1),')']);
  w = '';
else
